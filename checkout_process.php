<?php
    ...
    if (isset($_SESSION["uid"])) {
	    ...
	    (`order_id`,`user_id`,`f_name`, `email`,`address`,`city`, `state`,
        `zip`, `cardname`,`cardnumber`,`expdate`,`prod_count`,`total_amt`,`cvv`) 
	    VALUES ($order_id, '$user_id','$f_name','$email', 
        '$address', '$city', '$state', '$zip','$cardname','$cardnumberstr',
        '$expdate','$total_count','$prod_total','$cvv')";
        ...
        if(mysqli_query($con,$sql)){
            ...
            while($i<=$total_count){
                ...
                if(mysqli_query($con,$sql1)){
                    ...
                    echo"<script>window.location.href='http://www.sandbox.paypal.com/cgi-bgi/webscr'</script>";
                    }
                    else{
                    echo(mysqli_error($con));
                    }
